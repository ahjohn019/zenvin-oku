...
<div class="container" style="width:800px">
  ...
  <div class="panel-heading"><h3><strong>Add New Service </strong></div>
    {!! Form::open(array('route'=>'services.store', 'files'=>true)) !!}
    <div class="panel-body">
      @include('include.messages')
    </div>
        <div class="form-group">
            {{Form::label('serviceName', 'Name')}}
            {{Form::text('serviceName', '',['class' => 'form-control', 'placeholder' => 'Enter Service Name','required' => 'required'])}}
        </div>
        <div class="form-group" >
            {{Form::label('serviceDesc', 'Description')}}
            {{Form::textarea('serviceDesc', '',['class' => 'form-control', 'placeholder' => 'Enter service Description', 'rows' => '3','required' => 'required'])}}
        </div>
        <div class="form-group">
            {{Form::label('serviceTNC', 'Terms and conditions')}}
            {{Form::textarea('serviceTNC', '',['class' => 'form-control', 'placeholder' => 'Enter Terms and conditions', 'rows' => '3','required' => 'required'])}}
          </div>
          <div class="form-group">
            {{Form::label('serviceInstruction', 'Redeem instructions')}}
            {{Form::textarea('serviceInstruction', '',['class' => 'form-control', 'placeholder' => 'Enter Instruction', 'rows' => '3','required' => 'required'])}}
          </div>
          <div class="form-group">
            {{Form::label('serviceLocation', 'Location (address)')}}
            {{Form::textarea('serviceLocation', '',['class' => 'form-control', 'placeholder' => 'Enter Location', 'rows' => '2','required' => 'required'])}}
        </div>
          <div class="form-group">
            <table class="table">
              <tr>
                ...
                <td width="100px">{{Form::select('serviceType', [''=>'--Select Category--','MASSAGE' => 'MASSAGE','CLEANING' => 'CLEANING','TEACHING' => 'TEACHING','OTHERS' => 'OTHERS'],null,['class' => 'form-control'])}}</td>
              </tr>
              <tr>
                <td>{{Form::label('servicePrice', 'Price (MYR)')}}</td>
                <td>{{Form::text('servicePrice', '',['class' => 'form-control', 'placeholder' => 'Enter service Price', 'rows' => '1','required' => 'required'])}}</td>
